...
Because the ALB does not enforce IAM caller identity, the FastAPI application is the sole enforcement layer:
...
2. The API verifies the SigV4 signature and calls `sts:GetCallerIdentity` to resolve the caller's ARN.
3. The API calls `iam:ListGroupsForUser` to determine group membership. Result is cached in-process for 5 minutes per warm execution environment.
