...
**Negative / trade-offs**
- `POST /services/onboard` is slow (Terraform-backed provisioning takes 2–5 minutes). [ADR-014](ADR-014-services-onboard-provisioning-mode.md) commits v2 to a long-running synchronous request — clients must extend their default request timeout to cover the Lambda 15-min ceiling. The deferred async 202/`job_id` polling pattern is preserved in ADR-014 §"Deferred — Option B as v3" with a trip-wire metric, and is the planned next evolution if onboard p95 sustains past 10 minutes.
