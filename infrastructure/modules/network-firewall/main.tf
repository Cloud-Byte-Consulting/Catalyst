...
resource "aws_sns_topic" "egress_deny" { count = var.enabled ? 1 : 0 name = "${var.name_prefix}-egress-deny" }
...
resource "aws_networkfirewall_firewall_policy" "this" {
  ...
  firewall_policy {
    ...
    stateful_rule_group_reference { resource_arn = aws_networkfirewall_rule_group.fqdn[0].arn priority = 1 }
  }
}
...
resource "aws_networkfirewall_firewall" "this" {
  ...
  vpc_id              = var.vpc_id
  subnet_mapping { subnet_id = var.subnet_id }
}
