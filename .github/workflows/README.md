...
| `teardown.yml` | `workflow_dispatch` only (never push/schedule) | Human-gated destroy: operator must set `confirm_destroy=true` (or legacy `confirmation=destroy`) before any AWS call. Destroys all Terraform-managed resources; preserves bootstrap tier. Resets `CATALYST_LAMBDA_IMAGE_SEEDED` to `false`. See [`docs/teardown.md`](../../docs/teardown.md). | `AWS_ROLE_APPLY_ARN` |
| `teardown-scheduled.yml` | `schedule: cron(0 22 * * 1-5)` Mon–Fri 22:00 UTC + dispatch | Automated EOD teardown: destroys all Terraform-managed resources on a timer so the demo environment does not accrue costs overnight. No confirmation gate. Emits pre-destroy manifest to job summary. Resets `CATALYST_LAMBDA_IMAGE_SEEDED` to `false`. See [`docs/teardown.md`](../../docs/teardown.md). | `AWS_ROLE_APPLY_ARN` |
